<?php
class Hienth_MegaMenu_Block_Function extends Mage_Core_Block_Template
{
    public function getChildMenu($menu,$level,$listMenu)
    {
        if($menu->level >= $level)
        {
            return;
        }
        if($this->hasChildMenu($menu->id))
        {
            echo "<ul>";
                foreach ($listMenu as $k => $value)
                {
                    if($value->parent_id == $menu->id)
                    {
                        echo "<li>";
                        echo "<a href='$value->link'>$value->name</a>";
                                $this->getChildMenu($value,$level,$listMenu);
                        echo "</li>";
                    }
                }
            echo "</ul>";
