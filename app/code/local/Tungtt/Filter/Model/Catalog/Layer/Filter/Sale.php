...
class Tungtt_Filter_Model_Catalog_Layer_Filter_Sale extends Mage_Catalog_Model_Layer_Filter_Abstract
{
    ...
    public function apply(Zend_Controller_Request_Abstract $request, $filterBlock)
    {
        ...
        if (!$filter || Mage::registry('tungtt_filter_filter')) {
            return $this;
        }
        ...
        if ($filter == self::FILTER_ON_SALE) {
            ...
            $stateLabel = Mage::helper('tungtt_filter')->__('On Sale');
        } else {
            $select->where('price_index.final_price >= price_index.price');
            $stateLabel = Mage::helper('tungtt_filter')->__('Not On Sale');
        }
        ...
        Mage::register('tungtt_filter_filter', true);
        return $this;
    }
    /**
     * Get filter name
    ...
    {
        return Mage::helper('tungtt_filter')->__('Sale');
    }
    /**
    ...
    protected function _getItemsData()
    {
        ...
        $data[] = array(
            'label' => Mage::helper('tungtt_filter')->__('On Sale'),
            'value' => self::FILTER_ON_SALE,
            'count' => $status['yes'],
        );
        $data[] = array(
            'label' => Mage::helper('anhnt9_filter')->__('Not On Sale'),
