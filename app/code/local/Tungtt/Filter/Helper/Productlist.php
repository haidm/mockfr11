...
class Tungtt_Filter_Helper_Productlist extends Mage_ConfigurableSwatches_Helper_Productlist
{
    /**
    ...
    public function convertLayerBlock($blockName)
    {
        if (Mage::helper('configurableswatches')->isEnabled()
                ...
                foreach ($layer->getFilterableAttributes() as $attribute) {
                    if (Mage::helper('configurableswatches')->attrIsSwatchType($attribute)) {
                        $block->getChild($attribute->getAttributeCode() . '_filter')
                            ->setTemplate('mytemplate/filter/configurableswatches/catalog/layer/filter/swatches.phtml');
                    }
                }
            }
            ...
            if ($stateRenderersBlock = $block->getChild('state_renderers')) {
                ...
                $swatchRenderer->setTemplate('mytemplate/filter/configurableswatches/catalog/layer/state/swatch.phtml');
