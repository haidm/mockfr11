...
class SM_Slider_Adminhtml_SliderController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function indexAction()
    {
        ...
        $this->_addContent($this->getLayout()->createBlock('sm_slider/adminhtml_silder'))
            ->renderLayout();
    }
    ...
    public function editAction()
    {
        ...
        $sliderImageModel = Mage::getModel('sm_slider/image')->getCollection();
        $sliderModel = Mage::getModel('sm_slider/slider');
        if ($sliderId) {
    ...
    public function saveAction()
    {
        ...
        if ($this->validate($postData) == 'true') {
            ...
            $slider = Mage::getModel('sm_slider/slider');
            $slider->setId($sliderId)->setName($sliderName);
            try {
                $slider->save();
                if ($sliderId) {
                    if ($oldListImage) {
                        $listSliderInList = Mage::getModel('sm_slider/listimage')
                            ->getCollection()
                            ->addFilter('slider_id', $sliderId);
