...
* 
*/
class Nhatbq_MegaMenu_adminhtml_MenuController extends Mage_Adminhtml_Controller_Action
{
	...
	public function indexAction()
    {
        ...
        $this->loadLayout()
            ->_setActiveMenu('nhatbq/megamenu/index')
        ...
        $this->_addContent($this->getLayout()->createBlock('nhatbq_megamenu/adminhtml_menuitem'))
            ->renderLayout();
    }
    /**
     * Add action
    ...
    public function newAction()
    {
        ...
        $this->_addBreadcrumb(
            ...
            ->_addContent($this->getLayout()
                ->createBlock('nhatbq_megamenu/adminhtml_menuitem_new')
    ...
    public function editAction()
    {
        ...
        $menuItemModel = Mage::getModel('nhatbq_megamenu/menuitem');
        $menuItemList = Mage::getModel('nhatbq_megamenu/menuitem')->getCollection();
        ...
        if ($menuItemId) {
            ...
            if (!$menuItemModel->getId()) {
                Mage::getSingleton('adminhtml/session')
                    ->addError(Mage::helper( 'core')->__('Menu item not exist!'));
                $this->_redirect('*/*/');
                return;
            }
        }
        ...
        $this->loadLayout();
        $this->_setActiveMenu('nhatbq/megamenu/index');
        $this->_addBreadcrumb('Mega menu', 'Mega menu');
        $this->_addBreadcrumb('Menu item', 'Menu item');
        $this->getLayout()->getBlock('head')->setCanLoadExtJs(true);
        $this->_addContent($this->getLayout()
            ->createBlock('nhatbq_megamenu/adminhtml_menuitem_edit'))
            ->_addLeft($this->getLayout()
                ->createBlock('nhatbq_megamenu/adminhtml_menuitem_edit_tabs'));
        $this->renderLayout();
    }
    /**
     * Save action
    ...
    public function saveAction()
    {
        ...
        if ($this->getRequest()->getParam('parent_id') != 0) {
            ...
            $parentLevel = Mage::getModel('nhatbq_megamenu/menuitem')
...
//            var_dump($postData);die;
    ...
    public function deleteAction()
    {
        $menuItemId = $this->getRequest()->getParam('id');
        try {
            Mage::getModel('nhatbq_megamenu/menuitem')
                ->setId($menuItemId)
                ->delete()
                ->save();
            return $this->_redirect('*/*/');
    ...
    private function validate($data)
    {
        ...
        if ($data['type'] == 'Custom link' || $data['type'] == 'Product link' || $data['type'] == 'Category link') {
            if (!Zend_Validate::is($data['parent_id'], 'Regex', array('/^[0-9]+$/'))) {
                $errors[] = Mage::helper('core')->__('Invalid parent id');
            }
            if ($data['type'] == 'Custom link') {
                if (!Zend_Validate::is($data['link'], 'Regex',
                    array('@^(https?|ftp)://[^\s/$.?#].[^\s]*$@'))) {
                    $errors[] = Mage::helper('core')->__('Invalid url');
                }
                if (!Zend_Validate::is($data['name'], 'Regex', array('/^[a-z A-Z 0-9]{1,15}$/'))) {
                    $errors[] = Mage::helper('core')->__('Invalid Name');
                }
                if ($data['id']) {
            ...
            if ($data['type'] == 'Product link') {
                if (!Zend_Validate::is($data['product_sku'], 'Regex', array('/^[a-z A-z 0-9]{0,50}$/'))) {
                    $errors[] = Mage::helper('core')->__('Invalid product sku');
                }
            }
            if ($data['type'] == 'Category link') {
                if (!Zend_Validate::is($data['category_id'], 'Regex', array('/^[0-9]+$/'))) {
                    $errors[] = Mage::helper('core')->__('Invalid category id');
                }
            }
            if ($errors) {
                return $errors;
            ...
            $errors[] = Mage::helper('core')->__('Invalid type');
            return $errors;
        }
    }
}