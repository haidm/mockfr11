...
* 
*/
class Nhatbq_Slider_Adminhtml_ImageController extends Mage_Adminhtml_Controller_Action
{
	...
	public function indexAction()
    {
        ...
        $this->_addContent($this->getLayout()->createBlock('nhatbq_slider/adminhtml_image'))
            ->renderLayout();
    }
    /**
     * Edit action
    ...
    public function editAction()
    {
        ...
        $sliderImageModel = Mage::getModel('nhatbq_slider/image');
        $sliderModel = Mage::getModel('nhatbq_slider/slider')->getCollection();
        if ($sliderImage) {
            $sliderImageModel->load($sliderImage);
            $listImage = Mage::getModel('nhatbq_slider/listimage')
                ->getCollection()
                ->join(array('slider' => 'nhatbq_slider/slider'),
            ...
            if (!$sliderImageModel->getId()) {
                Mage::getSingleton('adminhtml/session')
                    ->addError(Mage::helper('core')->__('Image not exist!'));
                $this->_redirect('*/*/');
                return;
            }
        }
        ...
        $this->loadLayout();
        $this->_setActiveMenu('nhatbq/slider');
        $this->_addBreadcrumb('Slider', 'Slider');
        $this->_addBreadcrumb('Image', 'Image');
        $this->getLayout()->getBlock('head')->setCanLoadExtJs(true);
        $this->_addContent($this->getLayout()
            ->createBlock('nhatbq_slider/adminhtml_image_edit'));
        $this->renderLayout();
    }
    /**
     * Save action
    ...
    public function saveAction()
    {
        ...
        if ($this->validate($postData) == 'true') {
            ...
            $image = Mage::getModel('nhatbq_slider/image');
            if (!empty($_FILES['image']['name']) || $_FILES['image']['name']) {
                try {
                    $path = Mage::getBaseDir('media') . DS . 'hung_bd' . DS;
                    unlink($path.Mage::getModel('nhatbq_slider/image')->load($imageId)->getName());
                    $newDir = 'nhat_bq';
            ...
            try {
                ...
                if ($imageId) {
                    if ($oldListImage) {
                        $listImageInList = Mage::getModel('nhatbq_slider/listimage')
                            ->getCollection()
                        ...
                        $deleteListImage = Mage::getModel('nhatbq_slider/listimage')
                            ->getCollection()
                    ...
                    if ($newListImage) {
                        foreach ($newListImage as $item) {
                            Mage::getModel('nhatbq_slider/listimage')
                                ->setImage_id($imageId)
                                ->setSlider_id($item)
                                ->save();
                        }
                    ...
                    $lastId = Mage::getModel('nhatbq_slider/image')->getCollection()->getLastItem();
                    if ($newListImage) {
                        foreach ($newListImage as $item) {
                            Mage::getModel('nhatbq_slider/listimage')
    ...
    public function deleteAction()
    {
        ...
        try {
            $image = Mage::getModel('nhatbq_slider/image')->load($imageId);
            $fileName = $image->getName();
            $path = Mage::getBaseDir('media') . DS . 'nhat_bq' . DS;
            ...
            Mage::getSingleton('adminhtml/session')
                ->addError(Mage::helper('core')->__('An error has occurred while deleting this Image.'));
        }
        $this->_redirect('*/*/');
    }
    /**
    ...
    private function validate($data)
    {
        ...
        if ($data['text']){
            if (!Zend_Validate::is($data['text'], 'Regex', array('/^[a-z A-Z 0-9]{1,200}$/'))) {
                $errors[] = Mage::helper('core')->__('Invalid Text');
            }
        }
        if ($data['id']) {
            if (!Zend_Validate::is($data['id'], 'Regex', array('/^[0-9]+$/'))) {
                $errors[] = Mage::helper('core')->__('Invalid id');
            }
        }
        if ($data['listimage']) {
            foreach ($data['listimage'] as $item) {
                if (!Zend_Validate::is($item, 'Regex', array('/^[0-9]+$/'))) {
                    $errors[] = Mage::helper('core')->__('Invalid slider');
                }
            }
        }
        if ($data['listslider']) {
