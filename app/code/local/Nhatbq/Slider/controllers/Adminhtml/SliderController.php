...
* 
*/
class Nhatbq_Slider_Adminhtml_SliderController extends Mage_Adminhtml_Controller_Action
{
	...
	public function indexAction()
    {
        ...
        $this->_addContent($this->getLayout()->createBlock('nhatbq_slider/adminhtml_silder'))
            ->renderLayout();
    }
    /**
     * Edit action
    ...
    public function editAction()
    {
        ...
        $sliderImageModel = Mage::getModel('nhatbq_slider/image')->getCollection();
        $sliderModel = Mage::getModel('nhatbq_slider/slider');
        if ($sliderId) {
            ...
            if (!$sliderModel->getId()) {
                Mage::getSingleton('adminhtml/session')
                    ->addError(Mage::helper('core')->__('Slider not exist!'));
                $this->_redirect('*/*/');
                return;
            }
        }
        ...
        $this->_addContent($this->getLayout()
            ->createBlock('nhatbq_slider/adminhtml_slider_edit'));
        $this->renderLayout();
    }
    /**
     * Save action
    ...
    public function saveAction()
    {
        ...
        if ($this->validate($postData) == 'true') {
            ...
            $slider = Mage::getModel('nhatbq_slider/slider');
            $slider->setId($sliderId)->setName($sliderName);
            try {
                $slider->save();
                if ($sliderId) {
                    if ($oldListImage) {
                        $listSliderInList = Mage::getModel('nhatbq_slider/listimage')
                            ->getCollection()
                            ->addFilter('slider_id', $sliderId);
        ...
        if ($data['listimage']) {
            foreach ($data['listimage'] as $item) {
                if (!Zend_Validate::is($item, 'Regex', array('/^[0-9]+$/'))) {
                    $errors[] = Mage::helper('core')->__('Invalid image');
                }
            }
        }
        if ($data['newimage']) {
